...
- Рендерит `src/features/auto/ui/AutoScreen.jsx` (обертка над legacy `src/screens/Auto.jsx`) и показывает отладочную строку `ID: {id}`.
...
- Текущие рабочие экранные компоненты:
  ...
  - `src/features/auto/ui/AutoScreen.jsx`
- В `src/screens/*` добавлены deprecated-маркеры для неиспользуемых legacy экранов (`Home`, `Profile`, `ProfileClient`, `NewCar`).
- Рекомендуется:
