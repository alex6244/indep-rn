...
| Каталог для ИИ | ~248 моделей (`get-cars-to-banners`), seed-офлайн, фильтр марка/бюджет |
...
|------|------|------------|
| **B1** | 3–5 дней | ~~`POST /chat`, `POST /leads`, sync каталога, CORS~~ **скелет готов**; RN → `EXPO_PUBLIC_AI_API_URL` |
