...
**Главный сценарий для демо ИИ (2–3 мин):** **сначала вход по SMS** → бургер → **Подбор с ИИ** → в шапке «Каталог: с сайта (248)» → запрос («белджи», «KIA до 2,5 млн») → выбрать 1–2 авто → телефон → заявка. Нужен `EXPO_PUBLIC_AI_API_URL=https://ai-api.n-avtosalon.ru` и на сервере `AI_API_AUTH_ME_URL=https://indep.su/api/v1.0/me`.
